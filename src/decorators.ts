    ...
    for (const editor of vscode.window.visibleTextEditors) {
        if (editor.document === d) {
            ...
            editor.setDecorations(aiGeneratedDecorator, updatedRanges.filter(range => range.getType() === ExtendedRangeType.AIGenerated).map(range => {
                return {
                    range: range,
                    hoverMessage: `AI Generated under ${range.getAuthor() ? (range.getAuthor() + "'s") : 'your'} control${range.getAiName() !== '' ? ` • ${range.getAiName()} (${range.getAiModel()})` : ''}${range.getAiExplanation() !== '' ? ` • Explanation of change "${range.getAiExplanation()}"` : ''} • Created at: ${new Date(range.getCreationTimestamp()).toLocaleString()}`,
                };
            }));
